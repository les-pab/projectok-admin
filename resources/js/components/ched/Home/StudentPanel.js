...
import { fetchStudents, fetchStudentsWithFilters } from "../Service";
...
const CustomToolbar = ({
    ...
    const handleApplyFilters = async () => {
        ...
        if (filters.age) {
            const split = filters.age.split(",");
            for (let count = 0; count < split.length; count++) {
                if (count === 0) {
                    formFilters.push(["age", ">=", split[count]]);
                } else {
                    formFilters.push(["age", "<=", split[count]]);
                }
            }
        ...
        if (filters.gender) {
            formFilters.push(["gender", "=", filters.gender]);
        }

        if (filters.school) {
            formFilters.push(["school", "=", filters.school]);
        }
        ...
        const response = await fetchStudentsWithFilters(form);
    ...
    if (isFiltered) {
        filteredData.map((student, index) => {
            csvRows.push({
                ...
                appointments: student.appointment.length,
            });
        });
    } else {
        students.map((student, index) => {
                            ...
                            <Grid item xs={8}>
                                ...
                                >
                                    <Select
                                        ...
                                        value={filters.age}
                                    >
                                        <MenuItem value="16,18">
                                            16 to 18
                                        </MenuItem>
                                        <MenuItem value="19,21">
                                            19 to 21
                                        </MenuItem>
                                        <MenuItem value="22">22 above</MenuItem>
                                    </Select>
                                </FormControl>
                            </Grid>
                        </Grid>
