...
import {
    fetchCounselors,
    fetchCounselorsWithFilters,
    updateCounselorCredentials,
    deleteCounselor,
} from "../Service";
...
const CustomToolbar = ({
    ...
    const handleApplyFilters = async () => {
        ...
        if (filters.last_name) {
            formFilters.push(["last_name", "like", `%${filters.last_name}%`]);
        }

        if (filters.first_name) {
            formFilters.push(["first_name", "like", `%${filters.first_name}%`]);
        }
        ...
        if (filters.hei) {
            formFilters.push(["hei", "=", filters.hei]);
        }
        ...
        const response = await fetchCounselorsWithFilters(form);
    ...
    const handleExportAsPDF = () => {
        ...
        const headers = [
            ...
            { text: "# of Appointments", style: "tableHeader" },
        ];
    ...
    const csvHeaders = [
        ...
        { label: "# of Appointments", key: "appointments" },
    ];
    ...
    if (isFiltered) {
        filteredData.map((counselor) => {
            csvRows.push({
                ...
                mname: counselor.middle_name,
                hei: counselor.hei,
                designation: counselor.designation,
                appointments: counselor.appointment.length,
            });
        });
    } else {
        counselors.map((counselor) => {
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <MenuItem value="Ifugao State University-Nayon Campus">
                                            Ifugao State University-Nayon Campus
                                        </MenuItem>
...
const CounselorOptionsModal = ({
    useStyles,
    counselorOptionsModal,
    handleCloseCounselorOptionsModal,
    handleOpenCounselorUpdateCredentialsModal,
    selectedRow,
    setCounselors,
}) => {
    const classes = useStyles();
    ...
    const handleOnDelete = async () => {
        ...
        const response = await deleteCounselor(selectedRow);
        switch (response.status) {
            case 200:
                setCounselors(response.data[0]);
                setLoading(false);
...
const CounselorUpdateCredentialsModal = ({
    useStyles,
    counselorUpdateCredentialsModal,
    handleCloseCounselorUpdateCredentialsModal,
    selectedRow,
    form,
    setForm,
    inputError,
    ...
    const handleOnSubmit = async (e) => {
        ...
        const formData = {
            counselor_id: selectedRow,
            username: form.username,
            password: form.password,
        };

        const response = await updateCounselorCredentials(formData);
        switch (response.status) {
            case 200:
                handleCloseCounselorUpdateCredentialsModal();
                break;
    ...
    return (
        <Modal
            open={counselorUpdateCredentialsModal}
            closeAfterTransition
            BackdropComponent={Backdrop}
            BackdropProps={{
                timeout: 500,
        ...
        >
            <Container maxWidth="sm">
                <Fade in={counselorUpdateCredentialsModal}>
                    <Card>
                        <CardContent>
                            <Grid container direction="column" wrap="nowrap">
                                <Grid item>
...
const CounselorPanel = ({
    ...
    const [counselorOptionsModal, setCounselorOptionsModal] = useState(false);
    const handleOpenCounselorOptionsModal = () => {
        setCounselorOptionsModal(true);
    };
    const handleCloseCounselorOptionsModal = () => {
        setCounselorOptionsModal(false);
    };

    const [
        counselorUpdateCredentialsModal,
        setCounselorUpdateCredentialsModal,
    ] = useState(false);
    const handleOpenCounselorUpdateCredentialsModal = () => {
        handleCloseCounselorOptionsModal(false);
        setCounselorUpdateCredentialsModal(true);
    };
    const handleCloseCounselorUpdateCredentialsModal = () => {
        setForm({
            username: "",
            password: "",
        });
        ...
        setInputError(false);
        setCounselorUpdateCredentialsModal(false);
        setCounselorOptionsModal(true);
    };
    ...
    return (
        <>
                ...
                >
                    <DataGrid
                        ...
                        onRowDoubleClick={(params, event) => {
                            ...
                            handleOpenCounselorOptionsModal();
                        }}
            ...
            <CounselorOptionsModal
                useStyles={useStyles}
                counselorOptionsModal={counselorOptionsModal}
                handleCloseCounselorOptionsModal={
                    handleCloseCounselorOptionsModal
                }
                handleOpenCounselorUpdateCredentialsModal={
